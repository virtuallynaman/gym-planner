...
import { TrainingPlan, UserProfile } from "../../types/index.ts";

export async function generateTrainingPlan(
    profile: UserProfile | Record<string, any>
): Promise<Omit<TrainingPlan, "userId" | "id" | "version" | "createdAt">> {
    ...
    try {
        ...
        const planResponse = JSON.parse(content);

        return formatPlanResponse(planResponse, normalizedProfile);
    } catch (error) {
...
function formatPlanResponse(aiResponse: any, profile: UserProfile): Omit<TrainingPlan, "userId" | "id" | "version" | "createdAt"> {
    const plan: Omit<TrainingPlan, "userId" | "id" | "version" | "createdAt"> = {
        overview: {
        ...
        weeklySchedule: (aiResponse.weeklySchedule || []).map((weekDay: any) => ({
            day: weekDay.day || "Day",
            focus: weekDay.focus || "Full Body",
